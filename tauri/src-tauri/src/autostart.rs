...
pub fn render_systemd_unit(ctl: &Path, config: &Path, stderr: &Path, pid_file: &Path) -> String {
    ...
    format!(
        "[Unit]\n\
         ...
         [Service]\n\
         Type=simple\n\
         ExecStart={ctl} --config {cfg} daemon start --pid-file {pid}\n\
         Restart=on-failure\n\
         RestartSec=5\n\
...
pub fn render_windows_vbs(ctl: &Path, config: &Path, stderr: &Path, pid_file: &Path) -> String {
    ...
    // 用 WshShell.Run 第 2 参数 = 0 隐藏窗口（避免桌面突然蹦黑 cmd 窗），第 3 参数 = False 不等待。
    // 路径里含双引号要倒成 "" 转义；用户名/盘符通常没引号，但稳妥起见处理。
    ...
    // 把 stderr 路径塞进命令行的 `>>` 重定向需要 cmd.exe 包一层，但 daemon 自己已经支持
    // omlctl --pid-file 写 pid；stderr 想落盘的话需要 omlctl 自己 redirect 或者外层 cmd 包一下。
    // 这里用 cmd /c 包一层，保留 stderr 到文件，与 macOS launchd 的 StandardErrorPath 等价。
    format!(
        "' Auto-generated by oh-my-lan desktop client.\r\n\
         ' 把本机 daemon 注册为开机自启（仅当前用户）。删除本文件即可关闭自启。\r\n\
         Set WshShell = CreateObject(\"WScript.Shell\")\r\n\
         cmd = \"cmd /c \"\"\"\"{ctl}\"\"\" --config \"\"{cfg}\"\" daemon start --pid-file \"\"{pid}\"\" 2>> \"\"{err}\"\"\"\"\"\r\n\
...
mod tests {
    ...
    fn windows_vbs_contains_pid_file_and_hidden_window() {
        ...
        // pid-file flag present and after `daemon start`
        let daemon_idx = vbs.find("daemon start").unwrap();
        let pid_idx = vbs.find("--pid-file").unwrap();
        assert!(daemon_idx < pid_idx);
        // CRLF line endings, since Startup folder pre-Vista preferred them
        assert!(vbs.contains("\r\n"));
    }
